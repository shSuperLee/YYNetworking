...
//  MUCache
//
//  Created by zhujiawei on 2019/7/8.
//  Copyright © 2019 zhujiawei. All rights reserved.
//
...
/**
 MUKVStorageItem is used by `MUKVStorage` to store key-value pair and meta data.
 Typically, you should not use this class directly.
 */
@interface MUKVStorageItem : NSObject
...
@end
/**
 Storage type, indicated where the `MUKVStorageItem.value` stored.
 ...
 * If you want to store large number of small datas (such as contacts cache),
 use MUKVStorageTypeSQLite to get better performance.
 * If you want to store large files (such as image cache),
 use MUKVStorageTypeFile to get better performance.
 * You can use MUKVStorageTypeMixed and choice your storage type for each item.
 
 See <http://www.sqlite.org/intern-v-extern-blob.html> for more information.
 */
typedef NS_ENUM(NSUInteger, MUKVStorageType) {
    
    /// The `value` is stored as a file in file system.
    MUKVStorageTypeFile = 0,
    
    /// The `value` is stored in sqlite with blob type.
    MUKVStorageTypeSQLite = 1,
    
    /// The `value` is stored in file system or sqlite based on your choice.
    MUKVStorageTypeMixed = 2,
};



/**
 MUKVStorage is a key-value storage based on sqlite and file system.
 Typically, you should not use this class directly.
 
 @discussion The designated initializer for MUKVStorage is `initWithPath:type:`.
...
@interface MUKVStorage : NSObject
...
@property (nonatomic, readonly) MUKVStorageType type;  ///< The type of this storage.
 ...
 */
- (nullable instancetype)initWithPath:(NSString *)path type:(MUKVStorageType)type NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 If the `type` is MUKVStorageTypeFile, then the item.filename should not be empty.
 If the `type` is MUKVStorageTypeSQLite, then the item.filename will be ignored.
 It the `type` is MUKVStorageTypeMixed, then the item.value will be saved to file
 system if the item.filename is not empty, otherwise it will be saved to sqlite.
 
 @param item  An item.
 @return Whether succeed.
 */
- (BOOL)saveItem:(MUKVStorageItem *)item;

/**
 Save an item or update the item with 'key' if it already exists.
 
 @discussion This method will save the key-value pair to sqlite. If the `type` is
 MUKVStorageTypeFile, then this method will failed.
...
/**
 ...
 @discussion
 If the `type` is MUKVStorageTypeFile, then the `filename` should not be empty.
 If the `type` is MUKVStorageTypeSQLite, then the `filename` will be ignored.
 It the `type` is MUKVStorageTypeMixed, then the `value` will be saved to file
 ...
 */
- (nullable MUKVStorageItem *)getItemForKey:(NSString *)key;
 ...
 */
- (nullable MUKVStorageItem *)getItemInfoForKey:(NSString *)key;
...
/**
 ...
 @param keys  An array of specified keys.
 @return An array of `MUKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<MUKVStorageItem *> *)getItemForKeys:(NSArray<NSString *> *)keys;
...
/**
 ...
 @return An array of `MUKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<MUKVStorageItem *> *)getItemInfoForKeys:(NSArray<NSString *> *)keys;
